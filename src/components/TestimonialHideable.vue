...
<template>
  <div
    class="testimonial"
    :class="{ hidden: hideOverflow && isLongTestimonial(), long: isLongTestimonial() }"
  >
    <div class="icon">
      <img
        v-if="showImage"
        :src="quoterInfo.profileImgUrl"
        aria-label="testimonial-quoter-icon"
        @error="
          () => {
            showImage = false
          }
    ...
    <div class="quote-wrapper" @click="handleOverflowHide">
      <div class="quote">
        <div
          v-if="isLongTestimonial() && hideOverflow"
          class="continue-cover"
          :aria-label="`${quote[0]} Expand to read more.`"
        >
          <div class="cover" :aria-label="`${quote[0]} (Expand to read more)`">
            <font-awesome-icon
              size="xl"
      ...
      <div class="quoter-info">
        <a :href="quoterInfo.linkedInUrl">
          <div class="quoter-name-role">
            ...
            <div class="company-info">{{ `${quoterInfo.role}, ${quoterInfo.company} ` }}</div>
          </div>
